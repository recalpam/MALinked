...
class Sitemap extends \BaseController {
  ...
  public function getIndex(){
	...
	foreach($data->students as $student){
		$sitemap->add(URL::to('/profiel/'. $student->group->slug .'/'. $student->slug), $student->info->updated_at, '0.6', 'daily');
	}

	// Do the studies
	foreach($data->studies as $study){
		$sitemap->add(URL::to('/opleiding/'. $study->slug), $study->updated_at, '0.9', 'monthly');
	}

	// Do the groups
	foreach($data->groups as $group){
		$sitemap->add(URL::to('/klas/'. $group->slug), $group->updated_at, '0.7', 'monthly');
	}
